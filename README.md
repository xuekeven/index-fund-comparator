...
“投资手册”用于保存不随单次交易变化的概念、规则和方法，例如利率体系、黄金、债券、国债逆回购和红利策略；“投资笔记”用于记录某个日期的观点、操作与复盘。手册文章保存在独立的 `knowledge_article` 表中，不会混入笔记时间线。

文章可以填写最后复核日期和参考资料，便于定期检查涉及市场数据、监管规则或收益假设的内容。

投资笔记的观点归纳、自我总结、观察计划与投资手册正文共用同一个 Markdown 渲染器，支持一至六级标题、无序和有序列表、引用、粗体、斜体、删除线、行内代码、围栏代码块、链接、图片、分隔线、任务列表、GFM 表格及 KaTeX 数学公式。安全 HTML 白名单包含 `<details>`、`<summary>`、`<kbd>`、`<mark>`、`<sup>`、`<sub>` 等常用标签；`<script>`、`<iframe>`、`<style>`、事件属性、任意内联 `style` 和未授权 class 会在渲染前被移除。可用的预设 class 为 `md-text-muted`、`md-text-green`、`md-text-red`、`md-align-center`、`md-align-right`、`md-small` 和 `md-callout`。投资手册会根据正文标题在桌面端自动生成右侧目录，支持点击定位和阅读位置高亮；窄屏下自动隐藏。文章操作栏固定在阅读区顶部，“编辑正文”可直接把内容区切换为 Markdown 源码编辑器并单独保存正文；完整“编辑”仍用于修改文章其他字段。表格在窄屏设备上可以横向滚动，图片不会超出内容区域，外部链接会在新窗口中安全打开。

“新建笔记”和“新建文章”按钮右侧的齿轮分别进入“投资笔记管理”和“投资手册管理”；当前可配置字段分别为笔记“来源”和手册“分类”，以后新增的页面级配置也统一放在对应管理界面中。选项保存在 `content_option` 表中，按当前用户、选项类型和显示顺序隔离；弹层支持新增、改名、删除和排序。删除或改名只影响后续表单选择，不会批量改写历史笔记或文章；编辑带有旧值的历史内容时，旧值仍会临时出现在下拉选项中。
...
同步命令可重复执行，但仍应避免本地和服务器同时运行。上交所脚本 A 与深交所脚本 C 每周更新基金列表和目标基金主数据；上交所脚本 B 和深交所脚本 D 在工作日更新详情数据；脚本 B 将上交所 IOPV 仅保存为行情参考值，并从证监会 EID 获取正式净值及其真实日期。场外脚本 E 每月 1 日从证监会公募基金目录筛选目标并更新基金和主份额；脚本 F 在周一至周五更新产品概要费率、季度规模、正式净值、申购状态及五个区间收益率。场外基金没有交易所交易价，不写入或计算偏离。上交所费率不再以上交所详情页为准；脚本 B/C/F 共用 `backend/app/sync/eid_disclosures.py` 的证监会披露 PDF 解析和费率入库逻辑。
